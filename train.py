...
from albumentations import (
    Compose, RandomRotate90, Flip, Resize, Normalize,
    ColorJitter, GaussianBlur, ElasticTransform, GaussNoise,  MultiplicativeNoise
)
...
from Seg_UKAN.network.SemiModel import SemiModel
# ARCH_NAMES = archs.__all__
# LOSS_NAMES = losses.__all__
ARCH_NAMES = dir(archs)
LOSS_NAMES = dir(losses)
...
def parse_args():
    ...
    # model
    parser.add_argument('--arch', '-a', metavar='ARCH', default='SemiModel')
    ...
    parser.add_argument('--no_kan', action='store_true', default=False)
...
def main():
    ...
    train_transform = Compose([
        ...
        MultiplicativeNoise(multiplier=(0.9, 1.1), elementwise=True, p=0.2),
    ...
    def tta_predict(model, image, device):
        ...
        for tta in tta_transforms:
            # 对输入图像做TTA变换
            augmented = tta(image=image)['image']
            augmented = torch.from_numpy(augmented).permute(2, 0, 1).unsqueeze(0).float().to(device)
            with torch.no_grad():
                pred = model(augmented)
            pred = pred.squeeze().cpu().numpy()

            # 反变换 (这里只演示flip的情况)
            if isinstance(tta, HorizontalFlip):
                pred = np.flip(pred, axis=1)
            elif isinstance(tta, VerticalFlip):
                pred = np.flip(pred, axis=0)
            elif isinstance(tta, Rotate):
                pred = np.rot90(pred, -1)  # 旋转反向
    ...
    for epoch in range(config['epochs']):
        ...
        pd.DataFrame(log).to_csv(f'{output_dir}/{exp_name}/log.csv', index=False)#表格保存
        ...
        if val_log['iou'] > best_iou:
            torch.save(model.state_dict(), f'{output_dir}/{exp_name}/model.pth')#权重保存
